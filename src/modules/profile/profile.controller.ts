...
class ProfileController {
    ...
    public createProfile = async (
        ...
        const userId = req.user.id;
        try {
            const createUserData: IProfile =
                await this.profileService.createProfile(userId, userData);
    ...
    public createExperience = async (
        ...
        try {
            const createUserData: IProfile =
                await this.profileService.addExperience(userId, data);
    ...
    public createEducation = async (
        ...
        try {
            const createUserData: IProfile =
                await this.profileService.addEducation(userId, data);
            res.status(201).json(createUserData);
        } catch (error) {
            next(error);
        }
    ...
    public unFriend = async (
        ...
        const toUserId: string = req.params.id;
        try {
            const profile = await this.profileService.unFriend(
                req.user.id,
                toUserId,
            );
            res.status(200).json(profile);

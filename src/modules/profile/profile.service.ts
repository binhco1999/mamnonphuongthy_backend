    ...
    public follow = async (fromUserId: string, toUserId: string) => {
        ...
        if (!toProfile) {
            throw new HttpException(
                400,
                'There is not profile for target user',
            );
        }
        ...
        if (
            ...
            toProfile.followers.some(
                (follower: IFollower) =>
                    follower.user.toString() === fromUserId,
            )
        ) {
            throw new HttpException(
                400,
        ...
        if (
            ...
            throw new HttpException(
                400,
                'You has been already followed this user',
            );
        }
    ...
    public acceptFriendRequest = async (
        ...
        if (
            ...
            currentProfile.friends.some(
                (follower: IFollower) =>
                    follower.user.toString() === requestUserId,
            )
        ) {
            throw new HttpException(400, 'You has already been friend');
        }
        ...
        if (!currentProfile.friend_requests)
            currentProfile.friend_requests = [];
